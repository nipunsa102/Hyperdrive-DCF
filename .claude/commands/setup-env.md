...
- Detects the project's configuration convention from `CONFIG_GUIDE.md`'s Quick Start + `TECHSTACK.md` + the promoted `src/`
...
1. **Detect the configuration format** by cross-referencing these sources, in priority order:

   - **`CONFIG_GUIDE.md`'s Quick Start** (FIRST — authoritative) — it records the convention detected at promotion: the **Format**, the **Production file path(s)**, and the **Template(s)** generated by `/promote-poc`. When present, these settle the format and expected paths outright.
   - **`TECHSTACK.md`** (fallback) — if it names a specific configuration mechanism (e.g., "uses dotenv for local config", "config via YAML at `config/*.yaml`"), use that where the Quick Start is missing or silent.
   - **Promoted source code** under `src/` (fallback) — scan for how configuration is actually read at runtime (examples: `process.env.X` implies dotenv-style env vars; `fs.readFileSync('config.yaml')` implies YAML; `toml.load()` implies TOML; `import settings from './config'` implies a module import). The reader's expectations define the ground truth where the sources above are silent.
...
2. **Discover config files at project root only.** Enumerate files at the project root (non-recursive — do NOT descend into subdirectories, do NOT look in `poc/`) that match the detected format's filename patterns.
   ...
   **Exclude template files** — any file whose name contains `.example`, `.sample`, `.template`, `example`, `sample`, or `template` as a segment or suffix (e.g., `.env.example`, `config.sample.yaml`, `settings.template.toml`). Templates are produced by `/promote-poc` as scaffolding and are not real configuration.
   ...
   - **Zero real config files found** → ERROR and STOP:
     ...
     Detected format (from CONFIG_GUIDE.md Quick Start + TECHSTACK.md + src/): [format]
...
1. **INVOKE `smoke-test-agent`** with:
   ...
   - Scope: app
   - Module ID: app (app-scope run — verify the whole promoted application, structural + functional)
   - Start command: [the project's dev command — from CONFIG_GUIDE.md Quick Start / the project manifest]
   - Expected routes: [primary routes / endpoints from architecture/architecture.md]
...
Exit code: 0 if overall READY; non-zero if any hard failure in Phase 1, 2, any Phase 4 service FAIL, or an unrecovered FAIL in Phase 5.

## Outputs

Files created or modified:
- `tracking/env-setup.md` — run log with per-phase status, service connectivity results, and remaining actions (append-only history; each run adds a new `## Run N: [timestamp]` block)
